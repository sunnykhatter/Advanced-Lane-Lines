...
def colorGradThresholdImage(img):
	...
	# Threshold: gradient
	sobelx = mag_thresh(img, sobel_kernel=3, mag_thresh=(150, 255)) # Take the derivative in x
	...
	combined_binary[(s_binary == 1) | (l_binary == 1) | (r_binary == 1) | (yellow == 1) | (white == 1)] = 1
...
plt.plot(histogram)
# plt.show()
cv2.imwrite("output_images/histogramOfLane.jpg", histogram);
...
output_clip = clip.fl_image(pipeline_image)
output_clip.write_videofile(video_challenge_output, audio=False)
